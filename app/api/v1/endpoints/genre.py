...
def create_genre(
    *,
    ...
    Create new author
    """
    genre = crud.genre.create(db=db, obj_in=genre_in)
    return genre
...
def update_author(
    *,
    ...
    Update Genre
    """
    genre = crud.genre.get(db=db, id=genre_id)
    if not genre:
        raise HTTPException(status_code=404, detail="Genre not found")
    genre = crud.genre.update(db=db, obj_in=genre_in)
    return genre
...
def delete_genre(
    *,
    ...
    if not genre:
        raise HTTPException(status_code=404, detail="Genre not found")
    genre = crud.genre.delete(db=db, id=genre_id)
    return genre
